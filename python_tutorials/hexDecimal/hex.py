####
# print({"placeholder: smaller characters"})
# for i in range(17):
...
#     if (remainder >= (base ** power)):
#         #get the remainder for the next calculation
#         remainder = remainder % (base ** power)
#         binaryPowerList.append(power)
#         #print(power)
#
# index = binaryPowerList[0]
# end = binaryPowerList.length()
...
for power in powers:
    #integer division of Input number by the bit position
    bit = x // power
    if bit != 0 or power == 1:
        printing = True
    if printing:
