...
loc = 1
while True:
    # locationExitKeys = locations[loc].
    availableExits = ", ".join(locations[loc].get("exits").keys())
    #or availableExits = ", ".join(locations[loc]["exits"].keys())

    #print(availableExits)

    print(locations[loc].get("desc"))
    #or print(locations[loc].["desc"])

    if loc == 0:
        break
    else:
        allExits = locations[loc].get("exits").copy()
        allExits.update(locations[loc].get("namedExits"))
        # or allExits = locations[loc]["exits"].copy()
